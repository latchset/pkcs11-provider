...
static int p11prov_hmac_get_ctx_params(void *ctx, OSSL_PARAM params[])
{
    ...
    rv = p11prov_digest_from_hmac(macctx->hmac, &digest);
    if (rv != CKR_OK || digest == CK_UNAVAILABLE_INFORMATION) {
    ...
    if (p) {
        rv = p11prov_digest_get_digest_size(digest, &digest_size);
        if (rv != CKR_OK || digest_size == 0) {
    ...
    if (p) {
        rv = p11prov_digest_get_block_size(digest, &block_size);
        if (rv != CKR_OK || block_size == 0) {
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_DIGEST);
            return RET_OSSL_ERR;
        }
                            ...
                            size_t keylen, P11PROV_OBJ **keyobj)
{
    ...
    if (macctx->session == NULL) {
        rv = p11prov_get_session(macctx->provctx, &slotid, NULL, NULL,
...
static int p11prov_hmac_set_ctx_params(void *ctx, const OSSL_PARAM params[])
{
    ...
    if (p) {
        ...
        if (rv != CKR_OK) {
            P11PROV_raise(ctx, rv, "Unknown digest");
            return RET_OSSL_ERR;
        }
        ...
        if (rv != CKR_OK || macctx->hmac == CK_UNAVAILABLE_INFORMATION) {
            P11PROV_raise(ctx, rv, "No associated HMAC mechanism");
            return RET_OSSL_ERR;
        }
...
static int p11prov_hmac_op_init(P11PROV_MAC_CTX *macctx,
                                const OSSL_PARAM params[])
{
    P11PROV_debug("hmac op_init (ctx:%p, params:%p)", macctx, params);
    CK_RV rv = CKR_OK;
    CK_MECHANISM mech = { 0 };
    ...
    if (!macctx->session) {
        rv = p11prov_try_session_ref(macctx->key, macctx->hmac, false, false,
                                     &macctx->session);
    }
    if (rv != CKR_OK) {
        P11PROV_raise(macctx->provctx, rv, "Failed to get session");
        return RET_OSSL_ERR;
    }

    rv = p11prov_hmac_set_ctx_params(macctx, params);
    if (rv != RET_OSSL_OK) {
        P11PROV_raise(macctx->provctx, rv, "Invalid params");
        return RET_OSSL_ERR;
    }
    ...
    rv = p11prov_SignInit(macctx->provctx,
                          p11prov_session_handle(macctx->session), &mech,
                          p11prov_obj_get_handle(macctx->key));
    if (rv != CKR_OK) {
        return RET_OSSL_ERR;
    }
...
static int p11prov_hmac_legacy_init(void *ctx, const unsigned char *key,
                                    size_t keylen, const OSSL_PARAM params[])
{
    ...
    if (key) {
        ...
        rv = import_mac_key(macctx, key, keylen, &macctx->key);
        if (rv != CKR_OK) {
            return RET_OSSL_ERR;
        }
    ...
    return p11prov_hmac_op_init(macctx, params);
}

static int p11prov_hmac_init_skey(void *ctx, void *keydata,
                                  const OSSL_PARAM params[])
{
    P11PROV_MAC_CTX *macctx = (P11PROV_MAC_CTX *)ctx;

    if (keydata) {
        macctx->key = p11prov_obj_ref((P11PROV_OBJ *)keydata);
    }
...
static int p11prov_hmac_update(void *ctx, const unsigned char *in, size_t inl)
{
    ...
    rv = p11prov_SignUpdate(macctx->provctx,
...
const OSSL_DISPATCH p11prov_hmac_mac_functions[] = {
    ...
    DISPATCH_MAC_ELEM(hmac, INIT, legacy_init),
#ifdef OSSL_FUNC_MAC_INIT_SKEY
    DISPATCH_MAC_ELEM(hmac, INIT_SKEY, init_skey),
#endif
