...
static CK_ATTRIBUTE *get_or_fetch_allowed_mechs(P11PROV_OBJ *obj)
{
    ...
    if (obj->class != CKO_PRIVATE_KEY) {
        P11PROV_OBJ *priv = p11prov_obj_find_associated(obj, CKO_PRIVATE_KEY);
        if (priv) {
            am = get_or_fetch_allowed_mechs(priv);
    ...
    ret = p11prov_try_session_ref(obj, 0, false, false, &session);
    if (ret != CKR_OK) {
        return NULL;
    }

    {
        ...
        ret = p11prov_GetAttributeValue(obj->ctx, sh, handle, &attr, 1);
        if (ret == CKR_OK || ret == CKR_ATTRIBUTE_TYPE_INVALID) {
            if (attr.ulValueLen > 0
                && attr.ulValueLen != CK_UNAVAILABLE_INFORMATION) {
                attr.pValue = OPENSSL_malloc(attr.ulValueLen);
                if (attr.pValue) {
                    ret = p11prov_GetAttributeValue(obj->ctx, sh, handle,
                                                    &attr, 1);
                    if (ret == CKR_OK) {
        ...
        p11prov_clear_last_error_mark(obj->ctx);
    }
