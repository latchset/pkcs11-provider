...
#define FILL_KNOWN_SECRETS(alloc) \
    FILL_SECRET(GENERIC_SECRET, alloc), FILL_SECRET(AES, alloc), \
        FILL_SECRET(SHA_1_HMAC, alloc), FILL_SECRET(SHA256_HMAC, alloc), \
        FILL_SECRET(SHA384_HMAC, alloc), FILL_SECRET(SHA512_HMAC, alloc), \
        FILL_SECRET(SHA512_224_HMAC, alloc), \
                       ...
                       CK_OBJECT_HANDLE handle, P11PROV_OBJ *key)
{
     ...
     * here it is just a selector defalting to secret keys */
    ...
    case CKK_EC_EDWARDS:
        ...
        rv = pre_process_ec_key_data(key);
        break;
    case CKK_ML_DSA:
